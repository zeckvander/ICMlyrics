...
export default function AfinadorPanel({ onClose, minimized, setMinimized, isStacked }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      simInterval = setInterval(() => {
        if (cancelled) return;
        let freqBase = 442.5;
        if (cordaRef.current) freqBase = cordaRef.current.freqAlvo;
        else if (instRef.current !== "chromatic") freqBase = INSTRUMENTOS_PRESETS[instRef.current].cordas[0].freqAlvo;

        const oscilacao = (Math.sin(Date.now() / 600) * 5) + (Math.random() - 0.5) * 1.2;
        const freqSimulada = freqBase + oscilacao;
        ...
        if (cordaRef.current) {
          ...
          const nSim = 12 * (Math.log(freqSimulada / 440) / Math.log(2)) + 69;
          setCents(Math.round((nSim - nAlvo) * 100));
        } else {
          const { name, octave: oct, cents: ct } = freqToNote(freqSimulada);
          setNote(name); setOctave(oct); setCents(ct);
        }
        setFreq(freqSimulada); setActive(true);
      }, 100);
      return;

      /* CÓDIGO MICROFONE REAL (Descomente para produção):
      try {
        ...
        setLoading(false); detect();
      } catch (e) { setMicDenied(true); setLoading(false); }
      */
    }
    ...
    function detect() {
      ...
      if (detected > 25 && detected < 2000) {
        if (cordaRef.current) {
          setNote(cordaRef.current.nota); setOctave(cordaRef.current.oitava);
          const nAlvo = 12 * (Math.log(cordaRef.current.freqAlvo / 440) / Math.log(2)) + 69;
        ...
        setFreq(detected); setActive(true); lastDetectionTimeRef.current = Date.now();
      } else if (Date.now() - lastDetectionTimeRef.current > 800) setActive(false);
      rafRef.current = requestAnimationFrame(detect);
    }

    init();
    return () => { cancelled = true; clearInterval(simInterval); cancelAnimationFrame(rafRef.current); };
  }, []);
  ...
  if (minimized) {
    return (
      <div 
        className={`fixed ${isStacked ? "bottom-20" : "bottom-5"} left-5 z-50 bg-slate-900 text-white rounded-full shadow-lg flex items-center gap-2 pr-4 pl-3 py-2.5 border border-slate-800 transition-all duration-200`}
      >
        {/* Botão principal que ocupa todo o espaço do texto e maximiza ao clicar */}
        <button 
  ...
  return (
    <div className="fixed bottom-5 left-5 z-50 bg-slate-950 text-slate-100 rounded-2xl shadow-2xl border border-slate-800 w-80 overflow-hidden font-sans">
      ...
      <div className="p-4 space-y-4">
        ...
        {/* Botão de Sensibilidade Lado Esquerdo / Menor */}
        <div className="relative z-40">
            <button 
              onClick={() => setShowSensMenu(!showSensMenu)} 
              className="flex items-center gap-1 px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-md text-[10px] font-bold text-slate-400 hover:text-white transition uppercase tracking-wider"
            >
            ...
            {showSensMenu && (
                <div className="absolute top-full left-0 mt-1 flex flex-col bg-slate-900 border border-slate-800 rounded-lg p-1 min-w-[100px] shadow-xl">
                    {['pouco', 'normal', 'muito'].map(s => (
                        <button 
                          key={s} 
                          onClick={() => { setSensibilidade(s); setShowSensMenu(false); }} 
                          className={`w-full text-left px-2 py-1 rounded text-[10px] font-bold capitalize ${sensibilidade === s ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-400 hover:bg-slate-800'}`}
                        >
                          {s}
                        </button>
                    ))}
        ...
        <div className="bg-slate-900/50 rounded-xl p-3 text-center border border-slate-900">
            <p className={`text-[10px] font-black uppercase tracking-widest ${active ? (inTune ? "text-emerald-400" : "text-amber-400") : "text-slate-600"}`}>
                {!active ? (cordaSelecionada ? `Toque a corda ${cordaSelecionada.nota}` : "Aguardando som...") : inTune ? "Perfeito!" : cents > 0 ? `${cents} cents alto` : `${Math.abs(cents)} cents baixo`}
            </p>
        </div>
      </div>
    </div>
